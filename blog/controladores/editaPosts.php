...
<body>
    ...
    <div class="container pt-2">
        <h5>Editar post</h5>
        <?php $post = selectOne("posts",["Id" => $_GET["id"]]);
        ?>
        <form action="post.php" method="post" enctype="multipart/form-data">
            <div class="form-group"><input type="hidden" class="form-control" name="Id" readonly value="<?php echo($_GET['id']);?>"></div>
            <div class="form-group"><input type="text" class="form-control" name="Titulo" placeholder="Titulo" value="<?php echo($post["Titulo"]);?>" required></div>
            <div class="form-group"><textarea class="form-control" name="Corpo" placeholder="Corpo" rows="10" id="corpo" required><?php echo($post["Corpo"]);?></textarea></div>
            <div class="custom-file"><input type="file" class="custom-file-input" id="customFile" name="Imagem"><label class="custom-file-label" for="customFile"><?php echo($post["Imagem"]);?></label></div>
            <button type="submit" name="edita-post" class="btn btn-danger mt-2">Editar</button>
        </form>
    </div>
